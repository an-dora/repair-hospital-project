...
namespace App.Web.Controllers
{
	public class RoleController : AppControllerBase
	{
		...
		public RoleController(GenericRepository repository, IMapper mapper, MemoryCacheTicketStore memoryCacheTicketStore) : base(mapper)
		{
		...
		public async Task<IActionResult> Edit(RoleEditVM model)
		{
			...
			// Đăng xuất các user có role được update
			if(model.IsRequiredLogout)
			{
				var usersIdInRole = await _repository
										.GetAll<AppUser, string>(u => u.AppRoleId == role.Id,
												u => u.Id.ToString())
										.ToListAsync();
				foreach(var item in usersIdInRole)
				{
					await _memoryCacheTicketStore.RemoveAsync(item);
				}
				var currentUserRole = Convert.ToInt32(HttpContext.User.FindFirstValue(AppClaimTypes.RoleId));
